...
fn test_view_crate(){
    ...
    let response = client.get(format!("{}/crates/{}", APP_HOST, test_crate["id"]))
        .send().unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    let test_crate_view: Value = response.json().unwrap();
    assert_eq!(test_crate_view, json!({
    ...
    let response = client.get(format!("{}/crates/{}", APP_HOST, invalid_crate_id))
        .send().unwrap();
...
fn test_update_crate(){
    ...
    let rustacean  = create_test_rustacean(&client);
    let test_crate = create_test_crate(&client, &rustacean);
    let response = client.put(format!("{}/crates/{}", APP_HOST, test_crate["id"]))
        .json(&json!({
            "id": test_crate["id"],
...
fn test_delete_crate(){
    ...
    let rustacean  = create_test_rustacean(&client);
    let test_crate = create_test_crate(&client, &rustacean);
    let response = client.delete(format!("{}/crates/{}", APP_HOST, test_crate["id"]))
        .send().unwrap();
